...
-- /////////////////////////////////////////////////////////////////////////////////
-- Bagian Pencatatan Penjual (REDEFINISI - KODE DIPERBAIKI)
-- /////////////////////////////////////////////////////////////////////////////////

-- 1. Hapus tabel penjualan lama (Jika ada)
DROP TABLE IF EXISTS public.penjualan CASCADE;

-- 2. Buat ulang tabel 'penjualan'
CREATE TABLE IF NOT EXISTS public.penjualan (
    id UUID DEFAULT gen_random_uuid() PRIMARY KEY,
    
    -- Foreign Key ke Profile Penjual (Owner pencatatan)
    penjual_id UUID REFERENCES public.profile_penjual(id) ON DELETE RESTRICT NOT NULL,

    -- Foreign Key ke Produk yang dijual
    produk_id UUID REFERENCES public.produk(id) ON DELETE RESTRICT NOT NULL,

    -- Kolom Data Penjualan
    tanggal DATE NOT NULL DEFAULT CURRENT_DATE,
    nama_produk_history TEXT NOT NULL, 
    jumlah INTEGER NOT NULL CHECK (jumlah > 0),
    harga_satuan NUMERIC(10, 2) NOT NULL CHECK (harga_satuan >= 0),
    total_harga NUMERIC(10, 2) NOT NULL CHECK (total_harga >= 0),
    
    -- Kolom Tambahan dari skema lama
    kategori TEXT NOT NULL, 
    nama_pembeli TEXT NOT NULL,
    metode_pembayaran TEXT NOT NULL,
    ...
    -- Timestamp
    created_at TIMESTAMPTZ DEFAULT NOW()
    -- !!! PERHATIAN: Perintah CREATE INDEX DIHAPUS dari sini !!!
); 
-- !!! END CREATE TABLE !!!
...
-- 3. Pindahkan Perintah CREATE INDEX ke LUAR definisi tabel
CREATE UNIQUE INDEX IF NOT EXISTS penjualan_penjual_produk_idx 
ON public.penjualan (penjual_id, produk_id, created_at);

-- 4. Lanjutkan dengan RLS dan Trigger (Tidak ada perubahan pada bagian ini)
-- (Pastikan RLS dan Trigger yang Anda definisikan sebelumnya dijalankan setelah bagian ini)

-- RLS Penjualan: Hanya Penjual yang bisa MENGELOLA pencatatan penjualan mereka sendiri
...
-- 5. Fungsi dan Trigger Pengurangan Stok (WAJIB DIIKUTI)
-- (Lanjutkan dengan skrip fungsi update_stok_after_penjualan dan trigger kurangi_stok_trigger yang sudah saya berikan sebelumnya.)
-- 1. Buat fungsi untuk mengurangi stok produk
CREATE OR REPLACE FUNCTION public.update_stok_after_penjualan()
RETURNS TRIGGER
LANGUAGE plpgsql
...
BEGIN
    ...
    -- 2. Validasi stok sebelum pengurangan
    IF current_stok < NEW.jumlah THEN
        -- Membatalkan INSERT karena stok tidak mencukupi
        RAISE EXCEPTION 'Gagal mencatat penjualan: Stok produk (ID: %) tidak mencukupi. Sisa stok: %', 
            NEW.produk_id, current_stok;
    END IF;

    -- 3. Lakukan pengurangan stok di tabel 'produk'
    UPDATE public.produk
    SET stok = stok - NEW.jumlah,
        updated_at = now()
    WHERE id = NEW.produk_id;

    -- 4. Jika UPDATE berhasil, kembalikan baris baru yang di-insert
    RETURN NEW;
END;
$$;

-- 2. Buat trigger yang memanggil fungsi di atas setelah INSERT ke tabel 'penjualan'
CREATE TRIGGER kurangi_stok_trigger
